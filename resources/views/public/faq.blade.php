...
<section class="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
    ...
    <div class="mt-10 space-y-4">
        @foreach([
            ...
            'Apakah saya bisa melapor tanpa datang langsung?' => 'Bisa. Pengaduan dapat dibuat secara online melalui formulir SIPPAK.',
            ...
            'Apakah layanan gratis?' => 'Ya. Layanan pengaduan SIPPAK tidak dipungut biaya.',
            'Bagaimana jika laporan darurat?' => 'Segera hubungi layanan darurat, call center, atau WhatsApp petugas.'
        ] as $q => $a)
            <details class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><summary class="cursor-pointer font-black text-slate-900">{{ $q }}</summary><p class="mt-3 text-slate-600">{{ $a }}</p></details>
        @endforeach
    </div>
</section>
@endsection
