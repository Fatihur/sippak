@extends('layouts.app')
@section('title','SILAPAK - Layanan Pengaduan PPA')
@section('content')
<section class="grid lg:grid-cols-2 gap-10 items-center py-10">
    <div>
        ...
        <p class="mt-5 text-lg text-slate-600">SILAPAK membantu masyarakat melaporkan kasus secara aman, cepat, dan terpantau. Identitas korban/pelapor dijaga kerahasiaannya.</p>
    ...
    <div class="card bg-gradient-to-br from-blue-600 to-blue-800 text-white">
        ...
        <div class="mt-6 space-y-4">
            @foreach(['Isi formulir pengaduan','Verifikasi OTP','Dapatkan nomor tiket','Pantau perkembangan laporan'] as $i => $alur)
                <div class="flex gap-4 items-start"><span class="grid h-9 w-9 shrink-0 place-items-center rounded-none bg-white/15 font-bold">{{ $i+1 }}</span><p>{{ $alur }}</p></div>
            @endforeach
        </div>
    </div>
</section>
